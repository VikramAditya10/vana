...
import { arrangements, getFingeringForNote, type Arrangement } from '../content/catalog';
...
export function Practice({ arrangement, settings, updateSettings, onSaved }: { arrangement: Arrangement; settings: Settings; updateSettings: (value: Partial<Settings>) => void; onSaved: () => void }) {
  ...
  const noteForFingering = snap.state === 'countIn'
    ? (arrangement.events.find(n => n.kind === 'note') ?? active)
    : (active?.kind === 'note' ? active : upcoming.find(n => n.kind === 'note') ?? active);

  const activeFingering = noteForFingering && noteForFingering.kind === 'note'
    ? getFingeringForNote(noteForFingering.swara, noteForFingering.alteration, noteForFingering.octave)
    : undefined;

  const activeNoteLabel = noteForFingering && noteForFingering.kind === 'note'
    ? `${noteForFingering.swara}${noteForFingering.alteration === 'komal' ? ' (komal)' : noteForFingering.alteration === 'tivra' ? ' (tivra)' : ''}${active?.kind !== 'note' ? ' (prepare)' : ''}`
    : undefined;
